...
| Auto-theme, off by default (`omacosy-auto-theme on`): the custom theme appears, and while one is on screen the terminal follows it — the background, the 16 colours, the Starship prompt, the directory colour in `ls`, `eza` and yazi, bat, delta and its line strips, fzf, lazygit, fastfetch, btop, Neovim, and the band behind tmux's bar. The 16 are derived from the wallpaper with the six meaning-carrying hues held fixed, so red stays red. A stock theme is never changed: the generated files go and every app shows its own colours. Windows already open are repainted. `applier` in `auto-theme.conf` hands the palette to another tool instead, so only one program ever writes the terminal config. [docs/terminal-theming.md](docs/terminal-theming.md) | `helper/term-palette.swift`, `bin/omacosy-auto-theme`, `bin/omacosy-custom-theme`, `config/nvim/omacosy-theme.lua`, `config/starship-omacosy.template.toml`, `zsh/zshrc` |
