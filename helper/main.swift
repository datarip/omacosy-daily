...
func ownedWallpaperRoots() -> [String] {
    ...
    var roots = [repo.appendingPathComponent("themes").path]
    // The custom theme: its wallpaper folder, and the derived themes whose
    // backgrounds/ link into it. omacosy-custom-theme owns the folder rule.
    roots.append(home + "/.local/state/omacosy/derived")
    let p = Process()
    p.executableURL = URL(fileURLWithPath: home + "/.local/bin/omacosy-custom-theme")
    p.arguments = ["dir"]
    let pipe = Pipe()
    ...
    if (try? p.run()) != nil {
        ...
        p.waitUntilExit()
        let dir = String(data: out, encoding: .utf8)?
            .trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        if p.terminationStatus == 0, !dir.isEmpty { roots.append(dir) }
    }
...
case "wallpaper":
    ...
    if args[2] == "resync" {
        ...
        var roots: [String]? = nil  // costs a subprocess, so only when a screen differs
        ...
        for screen in NSScreen.screens {
            ...
            if roots == nil { roots = ownedWallpaperRoots() }
...
case "split-hint":
    ...
    // (dwindle:split_width_multiplier, default 1.0). 1.4 is tuned for a
    // 3440-wide display, where the half-slot (1712x1389) is still wider
    // than tall and would go side-by-side twice before it ever stacked.
    // On a narrow display it instead stacks slots that are wider than
    // tall: a 500x441 slot is side-by-side at 1.0 (500 >= 441) and
    // stacked at 1.4 (500 < 617.4), which reads as a split direction
